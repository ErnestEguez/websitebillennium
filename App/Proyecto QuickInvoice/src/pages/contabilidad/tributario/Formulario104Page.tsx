...
interface Declaracion104 {
    ...
    created_at:       string
    // agregados desde join con detalle
    iva_pagar:        number
    credito_sig:      number
}
...
export function Formulario104Page() {
    const { empresaActiva } = useAuth()
    const navigate = useNavigate()

    const [año, setAño]               = useState(new Date().getFullYear())
    const [declaraciones, setDeclaraciones] = useState<Declaracion104[]>([])
    ...
    async function cargar() {
        ...
        // Cargar cabeceras + casilleros 699 y 700 para mostrar IVA pagar / crédito
        const { data, error: er } = await supabase
            ...
            .select(`
                ...
                lp_iva_104_detalle(casillero, valor_final)
            `)
        ...
        const rows: Declaracion104[] = (data ?? []).map((d: any) => {
            const det: { casillero: string; valor_final: number }[] = d.lp_iva_104_detalle ?? []
            return {
                ...d,
                iva_pagar:  det.find(x => x.casillero === '699')?.valor_final ?? 0,
                credito_sig: det.find(x => x.casillero === '700')?.valor_final ?? 0,
            }
        })
    ...
    async function nuevaDeclaracion(mes: number) {
        ...
        const { data, error: er } = await supabase
            ...
            .insert({
                ...
                año,
                mes,
                version_form: año >= 2025 ? 'v2025' : 'v2024',
                estado:       'borrador',
            })
            .select('id')
            .single()
    ...
    return (
        <div className="space-y-5 max-w-5xl">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Formulario 104 — IVA</h1>
                    <p className="text-slate-500 text-sm mt-0.5">
                        Declaración mensual de IVA ante el SRI
                    </p>
                </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-slate-50 border-b text-xs text-slate-500 uppercase tracking-wide">
                                ...
                                <th className="py-2 px-4 text-right">IVA a pagar</th>
                                <th className="py-2 px-4 text-right">Crédito sig. período</th>
                                ...
                                return (
                                    <tr key={d.id} className="border-b border-slate-100 hover:bg-slate-50">
                                        ...
                                        <td className="py-3 px-4 text-right font-semibold">
                                            {d.iva_pagar > 0
                                                ? <span className="text-red-600">{formatMoneda(d.iva_pagar)}</span>
                                                : <span className="text-slate-400">—</span>}
                                        </td>
                                        <td className="py-3 px-4 text-right">
                                            {d.credito_sig > 0
                                                ? <span className="text-emerald-600">{formatMoneda(d.credito_sig)}</span>
                                                : <span className="text-slate-400">—</span>}
                                        </td>
            ...
            <div className="text-xs text-slate-400 flex items-center gap-2">
                ...
                Los valores se calculan automáticamente desde Integración SRI (compras) e Integración QI (ventas).
                Puedes ajustar casilleros manualmente antes de generar el XML.
            </div>
        </div>
    )
}
