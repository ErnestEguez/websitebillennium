...
const SECCIONES = [
    { key: 'trabajo_servicios',   label: 'Derivadas del Trabajo y Servicios Prestados', color: 'bg-blue-700' },
    { key: 'bienes_servicios',    label: 'Por Bienes y Servicios',                      color: 'bg-indigo-700' },
    { key: 'regalias_comisiones', label: 'Regalías, Comisiones y Arrendamientos',       color: 'bg-purple-700' },
    { key: 'capital',             label: 'Relacionadas con el Capital',                 color: 'bg-fuchsia-700' },
    { key: 'totales',             label: 'Totales',                                     color: 'bg-slate-700' },
]

const SECCIONES_READONLY = new Set(['totales']) // se recalculan solos
...
export function Formulario103DetallePage() {
    ...
    async function cambiarEstado(nuevoEstado: string) {
        ...
        await supabase.from('lp_rf_103').update(campos).eq('id', id)
        await supabase.from('lp_rf_103_log').insert({
            declaracion_id: id, empresa_id: decl.empresa_id,
    ...
    async function descargarXml() {
        ...
        const nombre = `F103_${empresaActiva.ruc ?? 'RUC'}_${decl.año}${String(decl.mes).padStart(2,'0')}.xml`
    ...
    const totalRetenido = casilleros.find(c => c.casillero === '499')?.valor_final ?? 0
            ...
            <div className="flex items-center gap-3 flex-wrap">
                ...
                <div className="flex-1">
                    <h1 className="text-2xl font-bold text-slate-900">
                        Formulario 103 — {mesNombre(decl.mes)} {decl.año}
                ...
                <HelpButton pageKey="formulario-103" />
                {/* Resumen rápido */}
                {totalRetenido > 0 && (
                    <div className="card px-4 py-2 bg-purple-50 border-purple-200 text-center">
                        <p className="text-xs text-purple-500">Total retenido</p>
                        <p className="font-bold text-purple-700">{formatMoneda(totalRetenido)}</p>
                    </div>
                )}
            </div>
            ...
            {!enviado && (
                <div className="card p-4 bg-amber-50 border-amber-200 text-xs text-amber-700 flex gap-3">
                    ...
                    <div>
                        ...
                        La sección de Totales se recalcula automáticamente.
                    </div>
                </div>
            )}
