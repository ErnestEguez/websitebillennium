...
const IVA_OPCIONES = [0, 5, 15]  // porcentajes disponibles en Ecuador
...
export function InventarioPage() {
    ...
    const subtotalSinIva = productosIngreso.reduce((sum, p) => sum + p.cantidad * p.costo_unitario, 0)
    const totalIva       = productosIngreso.reduce((sum, p) => sum + Math.round(p.cantidad * p.costo_unitario * p.iva_porcentaje) / 100, 0)
                        ...
                        <div className="space-y-3">
                            {productosIngreso.map((prod, index) => (
                                <div key={index} className="flex gap-3 items-start p-3 bg-slate-50 rounded-lg">
                                    ...
                                    />
                                    <select
                                        value={prod.iva_porcentaje}
                                        onChange={e => actualizarProducto(index, 'iva_porcentaje', parseInt(e.target.value))}
                                    ...
                                    <div className="w-28 px-3 py-2 bg-slate-100 rounded-lg text-right font-medium text-sm">
                                        <div>${(prod.cantidad * prod.costo_unitario).toFixed(2)}</div>
                                        {prod.iva_porcentaje > 0 && (
                                            <div className="text-xs text-slate-500">
                                                +${Math.round(prod.cantidad * prod.costo_unitario * prod.iva_porcentaje) / 100} IVA
                                            </div>
                                        )}
                                    </div>
                                    <button
