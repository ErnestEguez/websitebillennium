-- ============================================================
-- fn_eliminar_compra: permitir saldo de kardex negativo al eliminar
-- ============================================================
-- Decisión del usuario (2026-09-07): al eliminar una compra para
-- corregirla y volver a ingresarla (ej. un número de retención mal
-- asignado), no importa que el saldo corrido de kardex quede
-- momentáneamente en negativo — se va a re-ingresar la compra correcta
-- de inmediato, lo que vuelve a cuadrar el saldo. El bloqueo anterior
-- ("el saldo quedaría negativo") impedía justo ese flujo de
-- corrección, que es records el motivo más común para eliminar una
-- compra ya usada.
--
-- CREATE OR REPLACE idéntico a 20260802c_perm_eliminar_compra.sql,
-- solo se quita el RAISE EXCEPTION por saldo negativo (se deja un
-- RAISE NOTICE informativo en su lugar, para que quede rastro en logs
-- sin bloquear la eliminación). El resto de bloqueos (pagos aplicados,
-- NC de proveedor asociada) se mantienen intactos.
-- ============================================================
    ...
    IF v_compra.tipo_compra = 'INVENTARIO' THEN
        ...
        LOOP
            ...
            LOOP
                ...
                -- Antes bloqueaba con RAISE EXCEPTION si el saldo quedaba
                -- negativo. Decisión 2026-09-07: se permite — el caso de uso
                -- típico es eliminar para corregir y re-ingresar de inmediato,
                -- lo que vuelve a cuadrar el saldo. Solo se deja constancia
                -- en el log del servidor.
                IF v_saldo_cant < 0 THEN
                    RAISE NOTICE 'fn_eliminar_compra: saldo de kardex del producto % queda en % (negativo) tras eliminar compra %', rec.producto_id, v_saldo_cant, p_compra_id;
                END IF;
