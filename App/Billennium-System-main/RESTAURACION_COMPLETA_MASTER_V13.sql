/* 
  ...
  Versión: 1.3 (20/01/2026) - CORREGIDO
  Objetivo: Recrear estructura y forzar columnas de multi-tenencia.
  ============================================================================
*/
...
-- 2. TABLAS BASE (Asegurar columnas)
...
-- Forzar columna empresa_id en clientes si no existe
DO $$ 
BEGIN 
...
-- Forzar columna empresa_id en articulos si no existe y ajustar PK
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='articulos' AND column_name='empresa_id') THEN
        ALTER TABLE articulos ADD COLUMN empresa_id uuid REFERENCES empresas(id);
        -- Recrear el Primary Key para incluir empresa_id si es necesario
...
-- Proformas
CREATE POLICY "proforma_all_policy" ON proforma_cabecera FOR ALL TO authenticated 
...
-- Pedidos
CREATE POLICY "pedido_all_policy" ON pedido_cabecera FOR ALL TO authenticated 
USING (vendedor_id = auth.uid()::text OR (is_admin() AND empresa_id = get_my_empresa_id()))
WITH CHECK (vendedor_id = auth.uid()::text OR (is_admin() AND empresa_id = get_my_empresa_id()));
