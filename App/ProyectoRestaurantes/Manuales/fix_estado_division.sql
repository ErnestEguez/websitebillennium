-- =============================================================
-- FIX: Los pedidos divididos ahora heredan el estado del pedido original
-- Bug 5: Pedidos creados por dividir_pedido siempre quedaban en 'pendiente'
-- 
...
BEGIN
    ...
    LOOP
        ...
        -- CAMBIO BUG 5: estado hereda del pedido original (antes era hardcoded 'pendiente')
        INSERT INTO public.pedidos (
            empresa_id, 
            mesa_id, 
            mesero_id, 
            estado,           -- <-- CORREGIDO: heredar estado del original
            total, 
            nombre_cliente_mesa, 
            ...
            v_pedido_origen.estado,  -- <-- CORREGIDO: antes era la literal 'pendiente'
            0,
            v_nuevo_pedido_json->>'nombre_cliente', 
            v_nuevo_pedido_json->>'identificacion_cliente',
            v_nuevo_pedido_json->>'email_cliente',
            TRUE, 
            v_nuevo_padre_id, 
            NOW()
...
-- Refrescar permisos
GRANT EXECUTE ON FUNCTION public.dividir_pedido(UUID, JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION public.dividir_pedido(UUID, JSONB) TO service_role;
