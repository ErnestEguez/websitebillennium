...
import { Plus, ArrowRightLeft, Loader2, AlertCircle, X, Download } from 'lucide-react'
...
import type { AnticipoProveedor, Proveedor, CuentaBancaria } from '../../types/finance'
...
type FormAnticipo = {
    proveedor_id: string; cuenta_bancaria_id: string; forma_pago: 'transferencia' | 'cheque'
    monto: number; fecha: string; referencia: string; concepto: string
}
...
export function AnticiposPage() {
    ...
    const [form, setForm] = useState<FormAnticipo>({
        proveedor_id: '', cuenta_bancaria_id: '', forma_pago: 'transferencia',
    ...
    async function cargar() {
        ...
        setLista(await anticipoService.listar(empresa.id))
    }
    ...
    function exportarExcel() {
        const filas = lista.map(a => ({
            'Proveedor':    (a as AnticipoProveedor & { proveedor?: { nombre_empresa: string } }).proveedor?.nombre_empresa ?? '',
            'Forma Pago':   a.forma_pago,
            'Fecha':        a.fecha,
            'Monto':        a.monto,
        ...
        XLSX.writeFile(wb, `Anticipos_${empresa?.ruc ?? ''}.xlsx`)
    }
    ...
    return (
        <div className="space-y-5 max-w-6xl">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Anticipos a Proveedores</h1>
            ...
            <div className="card overflow-hidden">
                <div className="bg-slate-700 px-5 py-3 text-white text-sm font-bold flex items-center gap-2">
                    <ArrowRightLeft className="w-4 h-4" />Anticipos ({lista.length})
                </div>
                {loading ? (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {lista.map(a => {
                                    const p = a as AnticipoProveedor & { proveedor?: { nombre_empresa: string } }
                                    return (
                                        <tr key={a.id} className={cn('border-b border-slate-100 hover:bg-slate-50', a.estado === 'anulado' && 'opacity-50')}>
                                            <td className="py-2.5 px-4 font-semibold text-slate-800">{p.proveedor?.nombre_empresa ?? '—'}</td>
                ...
                <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                        <div className="flex items-center justify-between px-6 py-4 border-b">
                            <h2 className="font-bold text-slate-900">Nuevo Anticipo</h2>
                        ...
                        <div className="flex justify-end gap-3 px-6 py-4 border-t bg-slate-50 rounded-b-2xl">
                            ...
                            <button onClick={guardar} disabled={saving} className="btn btn-primary gap-2">
                                {saving && <Loader2 className="w-4 h-4 animate-spin" />}Registrar anticipo
                            </button>
                        </div>
                    </div>
                </div>
