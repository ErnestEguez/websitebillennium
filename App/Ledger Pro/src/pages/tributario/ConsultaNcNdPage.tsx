...
export function ConsultaNcNdPage() {
    const { empresaActiva } = useAuth()

    const [año, setAño] = useState(new Date().getFullYear())
    const [mes, setMes] = useState(0)
    const [tipoFiltro, setTipoFiltro] = useState<'todos' | 'nota_credito' | 'nota_debito'>('todos')
    const [busqueda, setBusqueda] = useState('')

    const [datos, setDatos] = useState<NcNd[]>([])
    ...
    async function cargar() {
        ...
        let q = supabase
            ...
            .select('id,tipo,proveedor_ruc,proveedor_nombre,numero,clave_acceso,fecha_emision,base_cero,base_iva,iva,total')
    ...
    const nc   = filtradas.filter(r => r.tipo === 'nota_credito')
    const nd   = filtradas.filter(r => r.tipo === 'nota_debito')
    ...
    return (
        <div className="space-y-5 max-w-6xl">
            {/* Header */}
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Consulta de N/C y N/D</h1>
                <p className="text-slate-500 text-sm mt-0.5">
                    Notas de Crédito y Notas de Débito importadas desde el SRI
                </p>
            </div>
            ...
            <div className="card overflow-hidden">
                <div className="bg-slate-700 px-5 py-3 text-white font-bold text-sm flex items-center gap-2">
                    ...
                    N/C y N/D — {mes > 0 ? mesNombre(mes) : 'Año completo'} {año}
                ...
                {cargando ? (
                    ...
                    <div className="py-12 text-center text-slate-400">
                        ...
                        <p>Sin N/C o N/D para los filtros seleccionados.</p>
                            ...
                            <tfoot>
                                <tr className="bg-slate-50 border-t-2 font-semibold text-sm">
                                    ...
                                    <td className="py-2.5 px-3 text-right text-xs">
                                        {formatMoneda(filtradas.reduce((s, r) => s + r.base_cero, 0), sym)}
                                    </td>
                                    <td className="py-2.5 px-3 text-right text-xs">
                                        {formatMoneda(filtradas.reduce((s, r) => s + r.base_iva, 0), sym)}
                                    </td>
                                    <td className="py-2.5 px-3 text-right text-xs">
                                        {formatMoneda(filtradas.reduce((s, r) => s + r.iva, 0), sym)}
                                    </td>
                                    <td className="py-2.5 px-3 text-right">
                                        {formatMoneda(filtradas.reduce((s, r) => s + r.total, 0), sym)}
                                    </td>
                                    <td />
                                </tr>
                            </tfoot>
